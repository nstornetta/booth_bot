...
def help_them_out():
    ...
    `@booth_bot course_num 30000` -- Give basic info on the top-rated sections of course 30000 offered this term
    `@booth_bot instructor Kleymenova` -- Give basic info on the courses being taught by Professor Kleymenova this term
    """
...
def run_query_command(query_type, command_list):
    ...
    try:
        ...
        result_strings = ["""*{title} {section}. Taught by {instructor} on {time} at {location}.* \n\tRecommend rating: {recommend}. \n\tHours per week: {hours}. \n\tInteresting rating: {interesting}.""".format(title=x[0].title(), 
        section=x[1], instructor=x[2].title(), time=x[3], location=x[4], hours=x[5], interesting=x[6], recommend=x[7]) for x in result]
